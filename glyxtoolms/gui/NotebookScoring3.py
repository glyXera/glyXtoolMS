...
class NotebookScoring(ttk.Frame):
    ...
    def __init__(self, master, model):
        ...
        self.tree.bind("a", self.generateSeries)
    ...
    def generateSeries(self, event):
        ...
        if len(selection) != 1:
            return
        self.model.classes["SpectrumView"].generateSeries(None)
    ...
    def sortColumn(self, col):
        ...
        if col == "Is Glyco" or col == "Status":
            ...
            l = [(str(self.tree.item(k, "text")), k) for k in self.tree.get_children('')]
        else:
        ...
        for spec, spectrum in analysis.data:
            ...
            for feature in spectrum.features:
                for hit in feature.hits:
                    ident.add(hit.status)
            for hit_status in ["Accepted","Rejected","Unknown"]:
                if hit_status in ident:
                    break
            
            
            itemSpectra = self.tree.insert("", "end", text=hit_status,
            ...
            self.specToId[str(spectrum.index)] = itemSpectra
