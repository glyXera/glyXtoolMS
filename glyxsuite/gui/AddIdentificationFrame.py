...
import glyxsuite
from glyxsuite.gui import ConsensusSpectrumFrame3
...
class AddIdentificationFrame(Tkinter.Toplevel):
    ...
    def fillAvailableModifications(self):
        ...
        for mod in glyxsuite.masses.PROTEINMODIFICATION:
            content = glyxsuite.masses.PROTEINMODIFICATION[mod]
        ...
        for mod in self.ownModifications:
            ...
            comp = glyxsuite.masses.getModificationComposition(mod)
            mass = glyxsuite.masses.calcMassFromElements(comp)
            targetstring = "?"
            item = self.treeModLeft.insert("",
                                           "end",
                                           text=mod,
    ...
    def clickedTreeLeft(self, event):
        ...
        copypeptide.modifications = self.peptide.modifications + [(mod, -1)]
        positions = set()
        for modificationlist in glyxsuite.fragmentation.getModificationVariants(copypeptide):
...
class ElementCompositionFrame(Tkinter.Toplevel):
    ...
    def updateComposition(self):
        ...
        if composition != "":
            # parse composition again, and calulate mass
            comp = glyxsuite.masses.getModificationComposition(composition)
            mass = glyxsuite.masses.calcMassFromElements(comp)
